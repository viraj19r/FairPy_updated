...
class HonestMetric:
    ...
    def evaluateCausal(self, plot_results=False):
        ...
        generator = pipeline('text-generation', model = self.model_name, device=self.device, tokenizer=self.tokenizer)
        set_seed(42)
    ...
    def evaluateMasked(self, plot_results=False):
        nlp_fill = pipeline('fill-mask', model=self.model, tokenizer=self.tokenizer, top_k = self.k, device=self.device)
