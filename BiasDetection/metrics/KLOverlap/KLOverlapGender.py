...
class KLOverLapGender(KLOverlap.KLOverlap):
    ...
    def topk_overlap(self, model, tokenizer, embedding, device, transformer, P=None, A=None, k=50, isMasked=False, mask=''):
        ...
        male_sent = np.genfromtxt(sys.path[1]+"data/corpus_male_context.txt", dtype=str, delimiter="\n")
        female_sent = np.genfromtxt(sys.path[1]+"data/corpus_female_context.txt", dtype=str, delimiter="\n")
